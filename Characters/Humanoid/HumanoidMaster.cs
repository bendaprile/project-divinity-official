...
public class HumanoidMaster : MonoBehaviour
{
    ...
    public void MoveToDest(Vector3 dest, bool HighPriority, float speed = 3f)
    {
        if (MovementLocked)
        {
            return;
    ...
    public void SetMovementLocked(bool set)
    {
        if (set)
        {
            MoveToDest(transform.position, true); //Stop walking
        }
        MovementLocked = set;
    }

    public void DisplayExternalText(string text)
    {
        ID.ExternalDisplayText(text);
    }
