...
public class Melee1H : Weapon
{
    ...
    public override void Attack()
    {
        if (CanAttack)
        {
            ...
            if(energy.Drain_ES(false, energyCost))
            {
                ...
                isHeavy = false;
                CanAttack = false;
                playMove.MeleeAttack(true, selfForce);
    ...
    public override void Attack2()
    {
        if (CanAttack)
        {
            if(energy.Drain_ES(false, heavyEnergyCost))
            {
                Attack_helper();
                isHeavy = true;
                CanAttack = false;
    ...
    public override void AnimationTriggerAttack()
    {
        if(EnemyWeapon)
        {
            foreach(Transform child in enemyWeaponController.ReturnTiMR())
            {
                if (child)
                {
                    child.GetComponent<Health>().take_damage(basicAttackDamage);
                }
            }
        }
        else
        {
            if (isHeavy)
            {
                MC.MeleeContactFunc(true, heavyAttackForce, heavyAttackDamage * (1 + stats.ReturnDamageMult(DS)));
            }
            else
            {
                MC.MeleeContactFunc(false, basicAttackForce, basicAttackDamage * (1 + stats.ReturnDamageMult(DS)));
            }
        }
    }
