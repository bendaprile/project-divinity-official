    ...
    private void CustomAttack()
    {
        if (CheckAmmo())
        {
            if (energy.Drain_ES(false, energyCost)) //Like this because this uses energy
            {
                ...
                if (EnemyWeapon)
                {
                    AmmoMaster AM_temp = fireProjectile(CustomAngle()).GetComponentInChildren<AmmoMaster>();
                ...
                {
                    fireProjectile(CustomAngle()).GetComponentInChildren<AmmoMaster>().Setup(ReturnFinalDamage(damage), dt_in: DT);
                }
            }
        }
    }
