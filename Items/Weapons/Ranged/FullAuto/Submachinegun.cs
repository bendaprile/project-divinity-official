...
public class Submachinegun : FullAutoRanged
{
    ...
    protected override void PlayerCustomAttack()
    {
        if (CheckAmmo())
        {
            if (energy.Drain_ES(false, energyCost)) //Like this because this uses energy
            {
                ...
                fireProjectile(CustomAngle() + spread).GetComponentInChildren<AmmoMaster>().Setup(ReturnFinalDamage(damage), dt_in: DT);
            }
        }
    }
    ...
    protected override void EnemyCustomAttack()
    {
        ...
        AmmoMaster AM_temp = fireProjectile(CustomAngle() + spread).GetComponentInChildren<AmmoMaster>();
