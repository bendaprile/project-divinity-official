    ...
    public override void AnimationTriggerAttack()
    {
        ...
        for (int i = 0; i < ProjectileCount; ++i)
        {
            ...
            if (EnemyWeapon)
            {
                if (i == 0)
                {
                    AmmoMaster AM_temp = fireProjectile(projectileAngle, true).GetComponentInChildren<AmmoMaster>();
            ...
            else
            {
                if (i == 0)
                {
                    fireProjectile(projectileAngle, true).GetComponentInChildren<AmmoMaster>().Setup(ReturnFinalDamage(damage), dt_in: DT);
                }
                else
                {
                    fireProjectile(projectileAngle, false).GetComponentInChildren<AmmoMaster>().Setup(ReturnFinalDamage(damage), dt_in: DT);
                }
            }
