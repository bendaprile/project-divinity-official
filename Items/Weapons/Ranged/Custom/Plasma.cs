...
public class Plasma : Ranged
{
    ...
    public override void EndSustainedAttack(bool disableCalled = false)
    {
        if (charging)
        {
            ...
            fireProjectile(CustomAngle()).GetComponent<AmmoPlasma>().PlasmaSetup(final_dam, final_size, false);
            charge_time = 0;
    ...
    protected override void Update()
    {
        ...
        if (charging)
        {
            ...
            if (charge_time < MaxChargeTime)
            {
                ...
                if (!energy.Drain_ES(false, energyCost * Time.deltaTime / MaxChargeTime))
                {
                    EndSustainedAttack();
                }
            }
