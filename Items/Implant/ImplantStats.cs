...
public class ImplantStats : ItemMaster
{
    ...
    [SerializeField] private bool CombatSkillLock = false;
    [SerializeField] private CombatSkillsEnum CombatSkillLockType = CombatSkillsEnum.ranged_proficiency;
    [SerializeField] private int CombatSkillValue = 0;
    ...
    protected override void AdvStatsHelper(List<(string, string)> tempList)
    {
        ...
        if (CombatSkillLock)
        {
            tempList.Add((STARTUP_DECLARATIONS.CombatSkillsEnumReverse[(int)CombatSkillLockType] + " Requirement:", CombatSkillValue.ToString()));
        }
        ...
        {
            tempList.Add((STARTUP_DECLARATIONS.statsDescriptions[(int)attributeName].Item1 + " Increase:", value.ToString() + (isAdd ? "" : "%")));
        }
    }
    ...
    public bool CheckLocked()
    {
        ...
        if (SkillLock && PlayStats.ReturnNonCombatSkill(SkillLockType) < SkillLockValue)
        {
            return true;
        }

        if (CombatSkillLock && PlayStats.ReturnCombatSkill(CombatSkillLockType) < CombatSkillValue)
        {
            return true;
        }
