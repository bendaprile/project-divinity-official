...
public class AmmoMaster : MonoBehaviour
{
    ...
    public virtual void Setup(int dam, bool destEnem = true, DamageType dt_in = DamageType.Regular) //NEED to destroy the parent on_disable for special bullets that have their main logic on a child (None yet)
    {
        damage = dam;
        ...
        if (FlashEffect != null)
        {
            var flashInstance = Instantiate(FlashEffect, transform.position, Quaternion.identity, transform.parent);
